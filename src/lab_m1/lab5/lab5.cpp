...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(234.0f), glm::vec3(1, 0, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_1) && persp && fov > minFOV) {
        fov -= deltaTime * 10;
        projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, zNear, zFar);
    }

    if (window->KeyHold(GLFW_KEY_2) && persp && fov < maxFOV) {
        fov += deltaTime * 10;
        projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, zNear, zFar);
        std::cout << fov << std::endl;
    }

    if (window->KeyHold(GLFW_KEY_3) && !persp) {
        right -= 5 * deltaTime;
        left += 5 * deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    }

    if (window->KeyHold(GLFW_KEY_4) && !persp) {
        right += 5 * deltaTime;
        left -= 5 * deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    }

    if (window->KeyHold(GLFW_KEY_5) && !persp) {
        top -= 5 * deltaTime;
        bottom += 5 * deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    }

    if (window->KeyHold(GLFW_KEY_6) && !persp) {
        top += 5 * deltaTime;
        bottom -= 5 * deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    }
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_LEFT))
    {
        ...
        if (window->GetSpecialKeyState() == 0) {
            ...
            // variables for setting up the rotation speed.
            //camera->RotateFirstPerson_OY(sensivityOY * -deltaX);
            camera->RotateFirstPerson_Bonus(sensivityOX * -deltaX);
            camera->TranslateRight(sensivityOX * -deltaX * 2);
        }
    }
}
