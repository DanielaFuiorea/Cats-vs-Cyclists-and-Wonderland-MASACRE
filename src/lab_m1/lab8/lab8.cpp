...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    // Render the point light in the scene
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0.75, 0.75));
    }
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_E))
            lightPosition[0] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_Q))
            lightPosition[0] -= up * deltaTime * speed;

        // TODO(student): Set any other keys that you might need
        if (window->KeyHold(GLFW_KEY_I))
            lightPosition[1] -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_J))
            lightPosition[1] -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_K))
            lightPosition[1] += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_L))
            lightPosition[1] += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_O))
            lightPosition[1] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_U))
            lightPosition[1] -= up * deltaTime * speed;

        if (spotLight) {
            if (window->KeyHold(GLFW_KEY_Z)) cutOff++;
            if (window->KeyHold(GLFW_KEY_X)) cutOff--;
